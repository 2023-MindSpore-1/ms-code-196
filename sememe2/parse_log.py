...
parser.add_argument('--model', type=str, default='bert') # 加参数
...
with open(model + "_log.pkl",'rb') as f: # 加载log文件
...
for word, data in logger.word_dict.items(): # 读取log文件的词和数据
    true_label = np.array([x['real_sense'] for x in data])# 所有的真实意思
    select_label = np.array([x['select_sense'] for x in data])# 选择的意思
