...
class BertNetworkForMask(nn.Cell):
    def __init__(self, config, is_training, use_one_hot_embeddings=False):
        super(BertNetworkForMask, self).__init__()
        self.bert = BertMask(config, is_training, use_one_hot_embeddings)
    ...
    def construct(self,
        ...
        if token_type_id==None:
            token_type_id=ops.zeros_like(input_ids)
...
class BertotherFormask(nn.Cell):
    def __init__(self, config, is_training,use_one_hot_embeddings=False):
        super(BertotherFormask, self).__init__()
        self.lamb_m=BertNetworkForMask(config, is_training)
    def construct(self,
                  input_ids,
                  input_mask=None,
                  token_type_id=None,
        ...
        prediction_scores = \
            self.lamb_m(input_ids,masked_lm_positions=masked_lm_positions)
...
class BertNetworkWithLoss(nn.Cell):
    ...
    def construct(self,
        ...
        return self.cast(total_loss, mstype.float32),ops.stop_gradient(prediction_scores)
